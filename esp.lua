...
--Tables (modified: Added ItemBoxes)
local SettingsTable = {
	...
	ItemBoxes = {Enabled = false, Outline = true, Filled = true, Color = Color3.fromRGB(255, 255, 255), FilledColor = Color3.fromRGB(0, 0, 0)}, -- Added for item boxes
...
do
	...
	function Functions:CreateItemEsp(Model, Data)
		...
		ItemEspList[Model] = {NameText = Drawings.Name, DistanceText = Drawings.Distance, FilledBox = Drawings.FilledBox, OutlineBox = Drawings.OutlineBox, InnerBox = Drawings.InnerBox, OuterBox = Drawings.OuterBox, Data = Drawings.Data}
		...
		for i, v in pairs(ItemEspData) do
			...
			if ScreenGui ~= nil and Model ~= nil and Model:IsDescendantOf(game:GetService("Workspace")) then
				...
				if distance <= SettingsTable.GeneralSettings.ItemRenderDistance and OnScreen and not table.find(SettingsTable.ObjectIgnoreList, Model) then
					do --Boxes
						v.FilledBox.Position = UDim2.new(0, left, 0, top)
						if SettingsTable.ItemBoxes.Enabled and SettingsTable.ItemNames.Enabled then
							v.FilledBox.Size = UDim2.new(0, right - left, 0, bottom - top)
							v.OutlineBox.Size = UDim2.new(1, 0, 1, 0)
							v.OutlineBox.Position = UDim2.new(0, 0, 0, 0)
							v.OutlineBox.BackgroundTransparency = 1
					...
					do --Names/Distances
						...
						if SettingsTable.ItemDistances.Enabled then
							if SettingsTable.ItemNames.Enabled then
								...
								v.Distance.PositionAt = UDim2.new(0, v.FilledBox.Position.X.Offset + v.FilledBox.Size.X.Offset / 2, 0, v.FilledBox.Position.Y.Offset + v.FilledBox.Size.Y.Offset + v.Distance.TextBounds.Y / 1.5)
							end
							v.Distance.Text = math.floor(distance / Conversions[SettingsTable.ItemDistances.Conversion]) .. " (" .. SettingsTable.ItemDistances.Conversion .. ")"
							v.Distance.Visible = true
